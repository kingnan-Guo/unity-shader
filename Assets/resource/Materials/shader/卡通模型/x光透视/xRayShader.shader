Shader "Unlit/xRayShader"
{
    Properties
    {
        ...
        _Diffuse ("Color", Color) = (0, 0, 0, 0)
        _OutLine("OutLine", Range(0.0, 2)) = 0.1 // 描边宽度
        _OutLineColor("OutLineColor", Color) = (0, 0, 0, 0) // 描边 颜色

        _Steps("steps", Range(0, 30)) = 3 // 离散化参数
        ...
        // 边缘 
        _RimColor("RimColor", Color) = (0, 0, 0, 0) // 边缘 光 颜色
        _RimPower("RimPower", Range(0.001, 3)) = 0.2 // 边缘 光 强度
        _RimWidth("RimWidth", Range(0, 1)) = 0.5 // 边缘 光 宽度


        // x射线 边框
        _XRayColor("XRayColor", Color) = (0, 0, 0, 0) // 边缘 光 颜色
        _XRayPower("XRayPower", Range(0.001, 3)) = 0.2 // 边缘 光 强度

    }
    SubShader
    {
        Tags {
            "Queue"="Geometry+1000" // 在其他的 半透明物体之后 渲染
            "RenderType"="Opaque" 
        }
        LOD 100
        ...
        Pass{
            ...
            Tags {
                "ForceNoShadowCasting"="True"// 不需要阴影
            }


            Blend SrcAlpha One // 半透明
            Cull Back
            // Lighting Off
            ZTest Greater// 深度测试
            ZWrite Off// 关闭深度写入
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                fixed3 xrayColor = _XRayColor * pow((1- rim), 1/_XRayPower);// pow(x, y) x 的 y次方
                return float4(xrayColor, 1);


                // return _XRayColor * (1 - rim) * _XRayPower;
            }
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // 边缘 :  法线 与 视角方向的点积 如果等于 1 那么是 垂直 ，越接近 1 调度越接近 90 度
                fixed3 viewDir = normalize(UnityWorldSpaceViewDir(i.worldPos));// 视角方向
                float rim = dot(i.worldNormal, viewDir);
                fixed3 rimColor = _RimColor * pow((1- rim), 1/_RimPower);// pow(x, y) x 的 y次方
