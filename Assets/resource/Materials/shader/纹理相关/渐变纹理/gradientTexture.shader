    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata_base v)
            {
                ...
                // 让 外面的数值 可以影响到 uv
                // o.uv = v.texcoord.xy * _RampTex_ST.xy + _RampTex_ST.zw;// 纹理坐标 ; https://www.lfzxb.top/unity-shader-base-texcoordn/
                // 替换上面
                o.uv = TRANSFORM_TEX(v.texcoord, _RampTex);// 纹理坐标；也就是俗称的UV，是顶点数据的一部分； uv和有没有贴图没关系哦，uv是网格的属性
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // 半兰伯特 
                // halfLambert = 0.5 * dot(世界光源方向, 顶点法线) + 0.5
                fixed halfLambert = max(0, dot(worldLightDir, i.worldNormal)) * 0.5 + 0.5;

                // 纹理图片采样 =========== 
                // tex2D 根据uv坐标（float2）获取纹理（sampler2D）上对应位置的颜色值
                // tex2D(_Bump, IN.uv_Bump) 从_Bump纹理中提取法向信息； uv 现在是  半兰伯特的 
                // tex2D(_RampTex, fixed2(halfLambert, halfLambert)).rgb;// 采样之后的值要乘到 漫反射 里， 因为属于 M 物体的材质 
