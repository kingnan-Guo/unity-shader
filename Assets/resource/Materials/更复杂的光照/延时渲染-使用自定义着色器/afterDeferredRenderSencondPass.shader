Shader "Unlit/afterDeferredRenderSencondPass"
{
    Properties
    {
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (unity_v2f_deferred i) : SV_Target
            {
                ...
                fixed3 diffuse = lightColor * diffuseColor * max(dot(halfDir, worldNormal), 0);

                fixed3 specular = lightColor * specularColor * pow(max(dot(halfDir, worldNormal), 0), gloss);

                // fixed3 ambient = lightColor * diffuseColor * 0.1;

                fixed3 color =  specular + diffuse;
