Shader "Unlit/Reffection"
{
    Properties
    {
        ...
        _ReflectionAmount ("ReflectionAmount", Range(0, 1)) = 1//强度
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // 反射 采样 cubeMap 乘以 反射颜色
                fixed3 refection = texCUBE(_CubeMap, i.WorldRefl).rgb * _ReflectionColor;

                // 模型颜色， 环境光  + （diffuse 与 反射颜色的融合 ） _ReflectionAmount是反射强度
                fixed3 color = ambient + lerp(diffuse, refection, _ReflectionAmount);
