Shader "Custom/waterSurfaceShaderNormal"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        // waterLight 自定义 光照 替换 Standard ; BlinnPhong 替换 Standard
        // vertex 需要顶点 顶点着色器
        // alpha:fade 控制透明度
        // noshadow 关闭阴影
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            // 先采样 第一次
            float bumpOffset1 = tex2D(_NormalTex, IN.uv_NormalTex + float2(_WaterSpeed * _Time.y, 0));
            float bumpOffset2 = tex2D(_NormalTex, float2( 1-IN.uv_NormalTex.y, IN.uv_NormalTex.x) + float2(_WaterSpeed * _Time.y, 0));

            float offset = (bumpOffset1 + bumpOffset2) / 2;// 得到偏移的额移动

            // o.Normal = UnpackNormal(offset);
