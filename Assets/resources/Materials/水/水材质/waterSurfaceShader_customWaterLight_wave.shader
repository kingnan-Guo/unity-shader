Shader "Custom/waterSurfaceShader_customWaterLight_wave"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
            ...
            fixed4 waveColor = tex2D(_WaveTex, float(waveB + _WaveRange * sin(_Time.y * _WaveSpeed + noiseColor.r)) + offset);
            // 采样的 颜色值 做 sin 变化 ; sin(_Time.y * _WaveSpeed + noiseColor.r)) 是 -1 ~ 1 之间，所以 +1 除 2 是 0-1 之间
            waveColor.rgb *= (1- (sin(_Time.y * _WaveSpeed + noiseColor.r) + 1) / 2)  * noiseColor.r;

            // 第二次采样 ; _WaveDelta 是波浪的偏移量;采样 插值
            fixed4 waveColor2 = tex2D(_WaveTex, float(waveB + _WaveRange * sin(_Time.y * _WaveSpeed + _WaveDelta + noiseColor.r)) + offset);
            waveColor2.rgb *= (1- (sin(_Time.y * _WaveSpeed + _WaveDelta + noiseColor.r) + 1) / 2)  * noiseColor.r;
            ...
            o.Alpha = min(_DepthRange, deltaDepth) /_TranAmount;
        }
        ENDCG
    }
    FallBack "Diffuse"
